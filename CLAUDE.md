...
**Proactive updates are mandatory, not judgment-gated** — this policy is now global (see `~/.claude/CLAUDE.md`'s graphify section and `~/.claude/references/graphify-sidecar-workaround.md`), applying to every project, not just this one — do not duplicate the general rule here. Project-specific detail: at the completion point of any feature addition/modification, compare the changed `.gd`/`.tscn`/`.tres` files against `graphify-out/graph.json`'s `built_at_commit`/mtime via `git diff --name-only <that point> HEAD -- '*.gd' '*.tscn' '*.tres'`, and if anything is missing from the graph, run the sidecar-workaround update automatically (see `graphify-gdscript-workaround` memory for the exact mechanics) — no staleness filter, no waiting to be asked. Prefer a background agent so it doesn't block the conversation.