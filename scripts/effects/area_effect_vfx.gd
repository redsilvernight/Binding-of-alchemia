...
# --- FEU : burst rapide et asymetrique, braises qui montent, trace de brule au sol ---

func _play_fire(radius: float, damage: float) -> void:
	...
	_spawn_decal(texture_feu, _scale_for(radius * 0.55), Color(0.15, 0.05, 0.03), 0.2 + 0.25 * intensity, 1.0)
	_free_after(1.6)
...
# --- EXPLOSIF : flash + onde de choc courte, debris, pas de trace persistante ---

func _play_explosion(radius: float, damage: float) -> void:
	var intensity: float = _intensity(damage)
	_play_burst(texture_explosif, radius * 0.35, 0.6, 0.7 + 0.3 * intensity, 0.09)
	_play_ring(radius * 0.9, Color(1.0, 0.55, 0.2, 0.5 + 0.4 * intensity), 0.3)
...
# --- ELECTRIQUE : anneau d'arcs a la source, puis chaine d'eclairs vers les cibles ---

const LIGHTNING_SEGMENTS: int = 5
const LIGHTNING_JITTER: float = 10.0
const LIGHTNING_FLASH_DURATION: float = 0.16

func _play_lightning_chain(radius: float, damage: float) -> void:
	var intensity: float = _intensity(damage)
